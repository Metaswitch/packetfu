...
module PacketFu
  ...
  class PcapFile < Struct.new(:endian, :head, :body)
    ...
    #
    #   :filename
    #   :array      # Can either be an array of packet data, or a hash-value pair of Timestamp => Packet.
    ...
    def array_to_file(args={})
      ...
      arr.each_with_index do |p,i|
        if p.kind_of? Hash # Binary timestamps are included
          this_ts = p.keys.first.dup.to_s
          this_incl_len = p.values.first.to_s.size
          this_orig_len = this_incl_len
          this_data = p.values.first.to_s
        else # it's an array
